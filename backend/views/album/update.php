...
{
    $lang_model = \common\models\AlbumLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\AlbumLang();
        $action = "/album-lang/create";

    } else {

        $lang_model = \common\models\AlbumLang::findOne($lang_model->id);
        $action = "/album-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/album-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
