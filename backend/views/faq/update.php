...
$this->title = Yii::t('main', 'Update FAQ');
...
{
    $lang_model = \common\models\FaqLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\FaqLang();
        $action = "/faq-lang/create";

    } else {

        $lang_model = \common\models\FaqLang::findOne($lang_model->id);
        $action = "/faq-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/faq-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
