...
{
    $lang_model = \common\models\PostLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\PostLang();
        $action = "/post-lang/create";

    } else {

        $lang_model = \common\models\PostLang::findOne($lang_model->id);
        $action = "/post-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
    	'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/post-lang/_form', [
        	'id' => $language->id,
        	'model' => $lang_model,
        	'action' => $action,
        ]),
