...
class Admission extends \yii\db\ActiveRecord
{
    ...
    {
        return $this->hasMany(AdmissionLang::className(), ['admission_id' => 'id']);
    }
    ...
    {
        $q = AdmissionLang::find()->select('name')->where(['admission_id' => $this->id,'language_id' => Yii::$app->lang->id]);
        if ($q->exists()){
            $d = $q->one();
        } else {
            $d = AdmissionLang::find()->select('name')->where(['admission_id' => $this->id])->one();
        }
    ...
    {
        $q = AdmissionLang::find()->select('level_name')->where(['admission_id' => $this->id,'language_id' => Yii::$app->lang->id]);
        if ($q->exists()){
            $d = $q->one();
        } else {
            $d = AdmissionLang::find()->select('level_name')->where(['admission_id' => $this->id])->one();
        }
    ...
    {
        $q = AdmissionLang::find()->select('reception_days')->where(['admission_id' => $this->id,'language_id' => Yii::$app->lang->id]);
        if ($q->exists()){
            $d = $q->one();
        } else {
            $d = AdmissionLang::find()->select('reception_days')->where(['admission_id' => $this->id])->one();
        }
